  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <div style={{
        ...
        padding: '4rem 2.5rem', textAlign: 'center', color: 'white',
      }}>
        <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 700, marginBottom: '0.75rem' }}>
          Welcome to Your Travel Dashboard
        </h1>
        <p style={{ fontSize: '0.95rem', opacity: 0.8 }}>Plan smarter, travel better with Planova</p>
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '3rem' }}>
          {[
            ...
            },
            {
              ...
              desc: 'Access your current travel plans, find hotels, discover restaurants, and locate emergency services for your destinations.',
            },
          ].map((card) => (
            <div key={card.title} style={{
              background: 'white', padding: '2.5rem', cursor: 'pointer',
            ...
            >
              ...
              <p style={{ fontSize: '0.85rem', color: '#666', lineHeight: 1.7 }}>{card.desc}</p>
            </div>
          ))}
        </div>

        {/* ── Advanced Travel Guide ── */}
        <h2 style={{ fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.12em', marginBottom: '1.25rem', color: '#0d1b2a' }}>
          ADVANCED TRAVEL GUIDE
        </h2>
        <div style={{ background: 'white', border: '1px solid #eee', padding: '2rem', marginBottom: '3rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
            {[
              ...
              <div key={item.title}>
                <div style={{
                  width: '48px', height: '48px', background: '#f0f0f0', borderRadius: '4px',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem',
                }}>
                  {item.icon}
                </div>
                <h4 style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.1em', marginBottom: '0.5rem', color: '#0d1b2a' }}>
                  {item.title}
                </h4>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.12em', color: '#0d1b2a' }}>OFFERS & REVIEWS</h2>
          <a href="#" style={{ fontSize: '0.78rem', color: '#888', textDecoration: 'none', letterSpacing: '0.05em' }}>VIEW ALL →</a>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem', marginBottom: '3rem' }}>
          {[
            { img: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400', title: "Cox's Bazar Beach Resort", valid: 'Valid until June 30, 2026', discount: '30% OFF' },
            { img: 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=400', title: 'Sylhet Tea Garden Tour', valid: 'Valid until July 15, 2026', discount: '20% OFF' },
            { img: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=400', title: 'Maldives Luxury Package', valid: 'Valid until August 20, 2026', discount: '25% OFF' },
            { img: 'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=400', title: 'Dhaka Food Tour Special', valid: 'Valid until May 31, 2026', discount: '15% OFF' },
          ].map((offer) => (
            <div key={offer.title} style={{ background: 'white', border: '1px solid #eee', overflow: 'hidden', cursor: 'pointer' }}
              onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,0.08)'}
              onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
            >
              <div style={{ position: 'relative' }}>
                <img src={offer.img} alt={offer.title} style={{ width: '100%', height: '160px', objectFit: 'cover', display: 'block' }} />
                <span style={{
              ...
              <div style={{ padding: '1rem' }}>
                <p style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0d1b2a', marginBottom: '0.3rem' }}>{offer.title}</p>
                <p style={{ fontSize: '0.75rem', color: '#999' }}>{offer.valid}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.12em', color: '#0d1b2a' }}>POPULAR DESTINATIONS</h2>
          <a href="#" style={{ fontSize: '0.78rem', color: '#888', textDecoration: 'none', letterSpacing: '0.05em' }}>VIEW ALL →</a>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem', marginBottom: '3rem' }}>
          {[
            ...
            <div key={dest.name} style={{ background: 'white', border: '1px solid #eee', overflow: 'hidden', cursor: 'pointer' }}
              ...
              onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
            >
              <img src={dest.img} alt={dest.name} style={{ width: '100%', height: '180px', objectFit: 'cover', display: 'block' }} />
              <div style={{ padding: '1.25rem' }}>
                <p style={{ fontSize: '0.9rem', fontWeight: 700, color: '#0d1b2a', marginBottom: '0.3rem' }}>{dest.name}</p>
                <p style={{ fontSize: '0.75rem', color: '#888', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  {dest.location}
                </p>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0d1b2a' }}>From {dest.price}</span>
                  <span style={{ fontSize: '0.82rem', color: '#f59e0b', fontWeight: 600 }}>★ {dest.rating}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* ── Recent Activity + Trending Now ── */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
          {/* Recent Activity */}
          <div style={{ background: 'white', border: '1px solid #eee', padding: '1.75rem' }}>
            <h3 style={{ fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.1em', color: '#0d1b2a', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', paddingBottom: '1rem', borderBottom: i < 2 ? '1px solid #f0f0f0' : 'none', marginBottom: i < 2 ? '1rem' : 0 }}>
                <div style={{ width: '36px', height: '36px', background: '#f5f5f5', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>
                  {item.icon}
                </div>
                <div>
                  <p style={{ fontSize: '0.83rem', color: '#0d1b2a', fontWeight: 500 }}>{item.action}</p>
                  <p style={{ fontSize: '0.75rem', color: '#999', marginTop: '2px' }}>{item.sub}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', paddingBottom: '1rem', borderBottom: i < 2 ? '1px solid #f0f0f0' : 'none', marginBottom: i < 2 ? '1rem' : 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: '0.75rem', color: '#999' }}>{item.country}</p>
                </div>
                <span style={{ fontSize: '0.78rem', color: '#22c55e', fontWeight: 600 }}>{item.trend}</span>
              </div>
            ))}
          </div>
        </div>
